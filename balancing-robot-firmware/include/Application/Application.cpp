...
ControlOut ModeHandMotionHandler(float dt)
{
    ...
    DataHandler_Service_Update(dt, &u);

    ImuObs imu = PIDControl_Service_IMUHandle(dt);

    u.avoid_gain = IoHwAb_UltraSonic_IsTriggered();

    u.armed = armed;

    const bool nearlyStill =
        (fabsf(imu.omega_dps) < 5.0f) &&
        (fabsf(u.v_cmd_mm_s) < VMAX_MM_S * 0.05f) &&
        (fabsf(u.yaw_cmd_dps) < 3.0f);

    static uint32_t stillStart = 0;
    if (nearlyStill && u.link_ok)
    {
        if (stillStart == 0)
            stillStart = millis();
        else if (millis() - stillStart > 1200)
